...
input <- read.csv2("FeatureDistribution_LogFC_enrichment_H3.csv", encoding="UTF-8",sep="\t")
...
probabilities <- phyper(overlap.group1, group1, group2, overlap.group2, lower.tail=TRUE)
...
write.table(input,file="FeatureDistribution_LogFC_enrichment_phyper_H3.csv", quote=FALSE,row.names = FALSE,sep = "\t")
...
for(i in names(input.list)){
     g<-ggplot(input.list[[i]], aes(y = logFC, x=Feature ,size=as.numeric(Size)) )+
        ...
        scale_size(limits = c(0.001,0.7),range=c(0.5,15))+
